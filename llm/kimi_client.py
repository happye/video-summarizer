...
class KimiClient:
    def __init__(self, max_context_messages=30):
        self.messages = []
        self.max_retries = 5
        self.timeout = 180  # 增加到180秒，避免边界情况超时
        self.retry_delay = 5
        self.perf_tracker = PerformanceTracker()
        self.request_count = 0
        self.max_context_messages = max_context_messages  # 限制上下文消息数量
        self.context_summary = ""  # 存储上下文摘要
    ...
    def generate(self, prompt, reset_context=False):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                print(f"[PERF] Attempt {attempt + 1}/{self.max_retries}: Sending request...")
                ...
                print(f"[PERF] Connection error (attempt {attempt + 1}/{self.max_retries}): {str(e)}")
                if attempt < self.max_retries - 1:
