...
if ($runChoice -eq "2") {
    ...
    Write-Host
    Write-Host "==============================="
    Write-Host "Download Only Mode"
    Write-Host "==============================="
    Write-Host "Enter video URL to download, or q to exit"
    Write-Host

    while ($true) {
        $url = Read-Host "URL"

        if ([string]::IsNullOrWhiteSpace($url) -or $url -eq "q" -or $url -eq "quit" -or $url -eq "exit") {
            Write-Host "Bye!"
            break
        }
